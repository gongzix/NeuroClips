...
class NeuroclipsPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        keylatents: Optional[torch.FloatTensor] = None,
        ...
        **kwargs,
        ...
        keylatents = self.prepare_latents(
            batch_size * num_videos_per_prompt,
            num_channels_latents,
            video_length,
            height,
        ...
        #print(latents[0,0,0,0:8,0:8])

        init_timestep = min(int(num_inference_steps * low_strength), num_inference_steps)
        t_start = max(num_inference_steps - init_timestep, 0)
        #fore_steps = self.scheduler.timesteps[t_start:]
        #fore_timestep = fore_steps[:1].repeat(batch_size)
        steps = self.scheduler.timesteps[:t_start]
        latent_timestep = steps[:1].repeat(batch_size)

        #noise = torch.randn_like(keylatents)
        noise = torch.randn_like(latents)
        #keylatents = self.scheduler.add_noise(keylatents, noise, fore_timestep)
        #latents[:,0,:,:,:] = keylatents*0.75+latents[:,0,:,:,:]*0.25
        #latents = keylatents
